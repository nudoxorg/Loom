...
- loom_hooks_install(harness) — install Loom reminder hooks for "claude", "codex", "cursor", or "all". This updates only the selected harness's user-level configuration under the home directory and never writes project files. Existing root-agent reminders remain in place. Claude and Codex also get SubagentStart context. Cursor gets a subagentStart lifecycle hook plus a Task-scoped preToolUse hook that prefixes context for new local subagents without changing resumed tasks. Hooks never create claims or change Loom project state automatically.
